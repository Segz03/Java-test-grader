...
// a letter grade to each student. Outputting the information to 2 outgoing files one with the
// students name, grade, and letter. The other lists the errors in the user ID's
      ...
      for(int i = 0; i < tst.size() ; i++)
      {
         ...
         while(j < st.size() && (!found))
         {
            if (st.get(j).getID().equals(tst.get(i).getID()))
            {
               ...
               outFile.printf("%-15s  %-15s   %-6.2f  %-2s\n", fName, lName, stScore, stLetGrade);
               found = true;
            }
            j++;
